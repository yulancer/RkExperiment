...
internal sealed class SubscriptionStarter : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation("Consumer B subscribed to {MessageType}", nameof(DemoEvent));
...
internal sealed class DemoEventHandler : IHandleMessages<DemoEvent>
{
    ...
    public async Task Handle(DemoEvent message)
    {
        var instance = _configuration["Service:InstanceName"] ?? "consumer-b";
        _logger.LogInformation("B handled event seq={Sequence}, eventId={EventId}, instance={Instance}", message.Sequence, message.EventId, instance);
